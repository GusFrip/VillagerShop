...
public class ShopConfigMenu extends AbstractContainerMenu implements ScrollableStorageMenu {
    ...
    public static final int UPGRADE_COUNT = ShopBlockEntity.UPGRADE_SLOTS; // 7 (dont le slot admin caché)
    ...
    public static final int[] UPGRADE_XS = {26, 44, 62, 80, 98, 116, 152}; // immortalité, déplacement, communication, mémoire, accès, anti-explosion, admin (caché)
    public static final int[] UPGRADE_YS = {40, 40, 40, 40, 40, 40, 40};
    ...
    private final ItemStackHandler chestHandler;
    private final ItemStackHandler upgradeHandler;
    private final ItemStackHandler tradeHandler;
    ...
    /** Données d'ouverture envoyées au client (Fabric extended menu). */
    public record OpenData(BlockPos pos, String name) {
        public static final StreamCodec<RegistryFriendlyByteBuf, OpenData> STREAM_CODEC =
                StreamCodec.composite(
                        BlockPos.STREAM_CODEC, OpenData::pos,
                        ByteBufCodecs.STRING_UTF8, OpenData::name,
                        OpenData::new);
    }

    /** Constructeur CLIENT. */
    public ShopConfigMenu(int id, Inventory inv, OpenData data) {
        this(id, inv, null, data.pos(), data.name(), new SimpleContainer(GHOST_SLOTS),
                new ItemStackHandler(ShopBlockEntity.MAX_STORAGE), new ItemStackHandler(1), new ItemStackHandler(ShopBlockEntity.UPGRADE_SLOTS), new ItemStackHandler(2), new ItemStackHandler(1));
    }
    ...
    private ShopConfigMenu(int id, Inventory inv, @Nullable ShopBlockEntity be, BlockPos pos, String shopName,
                           Container templates, ItemStackHandler storage, ItemStackHandler chestHandler, ItemStackHandler upgradeHandler, ItemStackHandler saveHandler, ItemStackHandler tradeHandler) {
        super(ModMenus.SHOP_CONFIG, id);
        this.be = be;
        this.pos = pos;
