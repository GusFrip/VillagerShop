...
### Added
- **First Fabric 1.21.1 release.** Same features as the NeoForge build (requires Fabric API and Patchouli).
...
### Ported
- **First NeoForge 1.21.1 release.** Full port from Forge 1.20.1: new networking (payloads), Data Components for the save book (NBT is gone), new capability system, 1.21 datapack layout (recipes/loot tables/tags).
- Stock slots are capped at 64 (1.21 raised the internal slot limit to 99) and at each item's own max stack size.
